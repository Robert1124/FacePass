...
- Sensitive local recognition gates for lock-screen assist and approved administrator/System Settings prompt fill wait up to 10 seconds for the first accepted local match. After that, FacePass may take short immediate follow-up captures to collect the remaining required match before returning immediately. Camera capture still stops after success, timeout, cancellation, or failure.
