...
The expected official GitHub Release artifacts are `FacePass-<version>.dmg` and `FacePass-<version>.dmg.sha256`. The DMG must contain the Developer ID-signed and notarized Mac app, the DMG itself must be Developer ID-signed before DMG notarization, and the website-hosted Sparkle appcast at `/updates/appcast.xml` must reference that DMG. The release workflow is tag-triggered for `v*` tags, with explicit `workflow_dispatch` reserved for manual release runs. The workflow creates or updates the GitHub Release as a draft while assets and Sparkle metadata are still being generated, publishes the GitHub Release only after the release gate passes, then writes the generated appcast back to `website/updates/appcast.xml` on the repository default branch. The site deployment must publish that path to `https://facepass.app/updates/appcast.xml`.
